...
public class Metodos {
  /**
   * Método que ordena cadenas por su tamaño, del más largo al más corto.
   * 
  ...
  public static Stream<String> ordenar(List<String> list) {
    return list.stream().sorted(Comparator.comparingInt(String::length));
  }

  /**
   * Método que descarta todos los elementos menores a cero o mayores a nueve,y
   * regresa un Stream<String> con la representación en español de cada dı́gito
   * 
  ...
  public static Stream<String> descartar(List<Integer> list) {
    ...
    return list.stream().filter(n -> (n >= 0 && n < 10)).map(n -> numeros[n]);
  }

  /**
   * Método que recibe una cadena s y devuelve un Stream<String> infinito con
   * repeticiones de s
   * 
  ...
  /**
   * Método que recibe un entero y regresa una lista con 50 múltiplos aleatorios
   * 
   * @param k - entero
  ...
  public static List<String> multiplos(int k) {
    ...
    return multi.filter(m -> (m % k == 0)).limit(50).mapToObj(String::valueOf).collect(Collectors.toList());
  }
